...
`PersonalEvent` 属于用户，使用 UTC 起止时间、活动时区、每周星期和结束日期生成发生记录。`PersonalEventException` 以系列 ID 和原始当地日期唯一定位单次例外；即使改期到另一个月也通过例外补入查询。后续编辑事务性拆分系列，保留过去发生记录。日历生成时按当前课表时区分段，与课程和学业时段共同排布。

个人安排的分享由每条 `Share.include_personal` 控制，迁移默认 false；仅通过所属课表主人的写操作变更。公开控制器在通过原有分享可用性和密码校验后才加载允许的活动投影（名称、分类、时间、地点、颜色），不传备注、所有者信息或编辑链接。图片导出开关只作用于当前浏览器的本地渲染，默认 false；个人明细数据在认证页面投影时已移除备注。
